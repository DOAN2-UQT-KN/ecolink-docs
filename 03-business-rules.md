...
| BR-005 | Tài khoản status=2 (bị ban) hoặc status=3 (tổ chức chưa kích hoạt) không đăng nhập được, kể cả qua Google | sign-in, Google callback | 403 "Account banned" / "…has not been activated yet…" | `auth.service.ts > login()`, `ID/modules/oauth/google.service.ts > handleCallback()` |
...
| BR-008 | Refresh token phải: verify được, có bản ghi REFRESH chưa revoke/dùng/hết hạn, userId khớp, user chưa bị xoá. Mỗi lần refresh sẽ revoke token cũ (rotation) | `POST /auth/refresh-token` | 401 "Invalid refresh token" | `auth.service.ts > refreshAccessToken()` |
...
| BR-011 | Kích hoạt tài khoản tổ chức: token `ORG_ACCOUNT_ACTIVATION` còn hiệu lực, password ≥ 8. Kích hoạt xong thì status=1 | `POST /auth/activate-org-account` | 400 "Invalid or expired activation token" | `auth.service.ts > activateOrgAccount()` |
...
| BR-029 | Provision tài khoản tổ chức: idempotent theo applicationId; email (lowercase) đã có user khác → 409; tạo user ORG, status 3, role ORG_OWNER, activation token TTL 72h | `POST /internal/v1/users/provision-org-account` | 409 "An account already exists for this contact email" | `ID/modules/auth/auth.service.ts > provisionOrgAccount()` |
...
| BR-054 | Xác thực OTP: tối đa 5 lần sai (quá thì OTP bị huỷ); đúng thì cấp submission token (30 phút) và vô hiệu các token cũ | `POST /email-otp/verify` | 400 OTP_INVALID; 429 OTP_TOO_MANY_ATTEMPTS | `verifyOtp()` |
| BR-055 | Presign giấy tờ và nộp đơn cần submission token hợp lệ, chưa dùng | `POST /documents/presign`, `POST /` | 401 SUBMISSION_TOKEN_INVALID | `submission-token.middleware.ts > requireSubmissionToken()` |
| BR-056 | Giấy tờ: docType thuộc `ApplicationDocType`; chỉ nhận pdf/jpeg/png; ≤ 10MB; ≤ 5 file mỗi đơn; chữ ký upload hết hạn sau 15 phút | presign | 400 INVALID_INPUT; 422 ORGANIZATION_DOCUMENT_LIMIT | `organization-application.service.ts > presignDocument()`, `storage/cloudinary-document-storage.ts` |
| BR-057 | Phải đồng ý xử lý dữ liệu cá nhân (consent = true) | `POST /` | 400 "Consent to personal data processing is required" | `createApplication()` |
| BR-058 | orgType ∈ GOV, SCHOOL, CLUB, NGO, SOCIAL_ENTERPRISE | `POST /`, `PUT /:id` | 400 "Unknown organization type" | `createApplication()`, `updateApplication()` |
| BR-059 | profile bắt buộc có name và logoUrl; profile.contactEmail phải trùng email đã xác thực OTP; toạ độ (nếu có) phải trong khoảng hợp lệ | `POST /`, `PUT /:id` | 400 | `validateProfile()` |
| BR-060 | Có ≥ 1 kênh chính thức (FACEBOOK_PAGE / WEBSITE / ZALO_OA), URL bắt đầu bằng http(s):// | `POST /`, `PUT /:id` | 400 "At least one official channel is required" | `validateChannels()` |
| BR-061 | Mỗi email liên hệ chỉ được có **1 đơn đang mở** (SUBMITTED, UNDER_REVIEW, NEEDS_MORE_INFO). Không có ràng buộc ở DB | `POST /` | 409 ORGANIZATION_APPLICATION_ALREADY_OPEN | `organization-application.repository.ts > findOpenByContactEmail()` |
| BR-062 | Một người đại diện pháp lý (so theo hash số giấy tờ) chỉ đứng tên tối đa **3** đơn ở trạng thái mở hoặc APPROVED. Có thể nâng hạn mức qua `legalRepLimitOverride`, nhưng không có API để ghi giá trị này | `POST /`, `PUT /:id` | 422 LEGAL_REP_LIMIT_EXCEEDED | `assertLegalRepUnderLimit()`, `DC/organization-trust.ts > DEFAULT_LEGAL_REP_ORG_LIMIT` |
| BR-063 | Người đại diện (nếu khai): fullName và phone bắt buộc; idType ∈ CCCD/MSSV/PASSPORT/OTHER; idNumber ≥ 4 ký tự. Chỉ lưu hash và 4 ký tự cuối | `POST /`, `PUT /:id` | 400 | `validateLegalRepresentative()` |
| BR-064 | Giấy tờ đính kèm phải tồn tại và thuộc cùng email đã xác thực; tối đa 5 | `POST /`, `PUT /:id` | 404 ORGANIZATION_DOCUMENT_NOT_FOUND; 422 | `assertDocumentsOwnedBy()` |
| BR-065 | Mã đơn có dạng `ORG-` + 8 ký tự hex viết hoa, thử lại 5 lần nếu trùng | `POST /` | 409 | `createWithUniqueCode()` |
| BR-066 | Tracking token (180 ngày) gắn với email, cho phép xem, sửa và rút mọi đơn có cùng contactEmail | `GET/PUT /:id`, `POST /:id/withdraw` | 401 / 404 | `resolveTrackingToken()`, `loadForApplicant()` |
| BR-067 | Chỉ được sửa (nộp lại) và tải thêm giấy tờ khi đơn ở **NEEDS_MORE_INFO** | `PUT /:id`, `POST /:id/documents/presign` | 409 ORGANIZATION_APPLICATION_NOT_EDITABLE | `updateApplication()`, `presignDocumentForApplication()` |
| BR-068 | Không rút được đơn đã APPROVED, REJECTED hoặc WITHDRAWN | `POST /:id/withdraw` | 409 ORGANIZATION_APPLICATION_ALREADY_DECIDED | `withdrawApplication()` |

## 4. Thẩm định đơn và Blue Tick

| ID | Rule | Áp dụng ở đâu | Hệ quả khi vi phạm | File |
|---|---|---|---|---|
| BR-070 | Claim bị chặn nếu đơn đang UNDER_REVIEW và người claim khác reviewerId hiện tại | `PUT /admin/organization-applications/:id/claim` | 409 ORGANIZATION_APPLICATION_CLAIMED | `organization-application-admin.service.ts > claim()` |
| BR-071 | Đơn đã APPROVED, REJECTED hoặc WITHDRAWN không thao tác được nữa | claim, request-info, decision | 409 ORGANIZATION_APPLICATION_ALREADY_DECIDED | `loadOpen()` |
...
| ID | Rule | Áp dụng ở đâu | Hệ quả khi vi phạm | File |
|---|---|---|---|---|
| BR-080 | Tạo tổ chức trực tiếp chỉ qua API key nội bộ: name ≤ 200, logoUrl bắt buộc, contactEmail bắt buộc, ownerId là UUID | `POST /api/v1/organizations` | 401 / 400 | `INC/modules/organization/organization.controller.ts > createOrganization` |
...
| BR-083 | Chỉ owner được sửa tổ chức; phải có ít nhất 1 field; đổi contactEmail thì mất trạng thái xác minh email và hệ thống gửi link mới | `PUT /organizations/:id` | 403; 400 | `updateOrganization()` |
...
| BR-085 | Xin gia nhập: owner không được xin; người đã là thành viên không được xin; không được có 2 yêu cầu PENDING | `POST /organizations/:id/join-requests` | 400 / 409 | `createJoinRequest()` |
| BR-086 | Chỉ owner xem và xử lý yêu cầu gia nhập; yêu cầu phải đang PENDING | `GET /:id/join-requests`, `PUT /join-requests/process` | 403; 409 JOIN_REQUEST_ALREADY_PROCESSED | `processJoinRequest()`, `listJoinRequestsForOwner()` |
| BR-087 | Chỉ người xin được huỷ, và chỉ khi yêu cầu PENDING | `DELETE /join-requests/cancel` | 403 / 400 | `cancelJoinRequest()` |
| BR-088 | Owner không được rời tổ chức; phải là thành viên thì mới rời được | `DELETE /:id/members/me` | 400 | `leaveOrganization()` |
...
| BR-151 | Chỉ **owner của tổ chức** được tạo campaign; tổ chức phải tồn tại (không xét status) | tạo campaign | 404 / 403 "Only the organization owner can create campaigns" | `campaign.service.ts > createCampaign()` |
...
| BR-295 | Đơn tổ chức: OTP đúng 6 số; logo bắt buộc; kênh đầu tiên bắt buộc; không bắt buộc phải có ≥ 1 giấy tờ | `/organizations/apply` | Form báo lỗi | 06-frontend.md |

---

**Tổng số rule đã ghi nhận: 162** (BR-001…BR-295, có các khoảng trống dành sẵn trong từng dải).
