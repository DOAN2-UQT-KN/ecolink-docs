...
| POST | `/api/v1/organization-applications/:id/owners/:candidateId/resend` | Track, limiter | Chủ hòm mail | Param `id`, `candidateId` UUID | 200 `{ application }` | 401; 404; 409 APPLICATION_NOT_ACTIVE; 429 RESEND_TOO_SOON | `…controller.ts > resendOwnerInvite` → `resendOwnerInvite()` |
...
Rule xác nhận owner (`owner-confirmation.service.ts`): xem `03-business-rules.md` BR-308..BR-310. Gửi lại (`resendOwnerInvite()`): đơn AWAITING / NEEDS_REVISION, owner PENDING, `sentAt + 1h ≤ now` (không giới hạn số lần).
...
| Email `ORG_APPLICATION_DRAFT_STARTED` | HTTP | `{ toEmail, applicationCode, editUrl, expiresInDays, locale, appName }` | OTP mở đơn DRAFT mới (không gửi khi `resumed`) | notification-service | `enqueueApplicationDraftStartedEmail()` |
| Email `ORG_APPLICATION_DRAFT_UPDATED` | HTTP | `{ toEmail, applicationCode, organizationName, savedAt, editUrl, locale, appName }` | `PUT /:id` với `notify_submitter: true`, tối đa 1/giờ/hồ sơ (event `DRAFT_UPDATE_NOTIFIED`) | notification-service | `enqueueApplicationDraftUpdatedEmail()` |
